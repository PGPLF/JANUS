# Journal de bord du Projet JANUS

## 2025-01-05 - Initialisation du projet

### Actions réalisées

#### 1. Création de la structure du projet
- **10:41** - Création du dossier `JANUS` sur le bureau
- Création des sous-dossiers:
  - `JANUS-PUB_REF/` : Publications de référence sur le modèle JANUS
  - `JANUS-PUB_CRI/` : Publications et articles critiques du modèle

#### 2. Création du repository GitHub
- **10:45** - Repository créé: https://github.com/PGPLF/JANUS
- Visibilité: Public
- Branche principale: master
- Synchronisation locale/GitHub établie

#### 3. Téléchargement des publications de référence (11 PDFs)

| Fichier | Taille | Pages | Statut |
|---------|--------|-------|--------|
| Petit_2024_EPJ-C_Bimetric_Sakharov.pdf | 2.6 MB | - | OK |
| Petit_2021_Schwarzschild_singularity.pdf | 712 KB | 6 | OK |
| Debergh_2018_Dirac_negative_mass.pdf | 126 KB | 14 | OK |
| Petit_2016_JCM_French.pdf | 6.3 MB | - | OK |
| Petit_2014_Negative_mass_GR.pdf | 103 KB | 9 | OK |
| Petit_2007_Bigravity_VSL.pdf | 439 KB | 17 | OK |
| Henry-Couannier_2005_Twin_Universe.pdf | 167 KB | 14 | OK |
| Petit_1995_Twin_Universes.pdf | 519 KB | - | OK |
| Petit_2022_Sponsors.pdf | 903 KB | 8 | OK |
| Petit_HAL_Janus_Crisis_Cosmology.pdf | 2.1 MB | 8 | OK |
| Petit_HAL_Janus_Model.pdf | 3.3 MB | 44 | OK |

**Total: 11 fichiers PDF, ~17 MB**

#### 4. Téléchargement des publications critiques

| Fichier | Taille | Statut |
|---------|--------|--------|
| Riazuelo_IAP_Critique_JPP.html | 14 KB | OK |

#### 5. Création des fichiers INDEX
- `JANUS-PUB_REF/INDEX.md` : Index détaillé des publications avec DOI et références
- `JANUS-PUB_CRI/INDEX.md` : Index des critiques académiques

#### 6. Synchronisation GitHub
- **11:52** - Premier push avec toutes les publications
- Commit: "Ajout des publications de référence et critiques du modèle JANUS"

---

### Vérification des fichiers (2025-01-05)

Tous les fichiers PDF ont été vérifiés et sont valides:
- 11 PDFs dans JANUS-PUB_REF/ : tous au format PDF valide
- 1 HTML dans JANUS-PUB_CRI/ : critique de Riazuelo (IAP)

---

## 2025-01-05 - Intégration des publications de JANUS-Z

### Découverte du dossier JANUS-Z existant
- Le dossier `/Desktop/JANUS-Z/` contient déjà un projet JANUS plus avancé
- Contient: scripts, données, résultats, analyses
- Sous-dossier `publications_ext/` avec des publications supplémentaires

### Publications récupérées de JANUS-Z

| Fichier | Taille | Source | Statut |
|---------|--------|--------|--------|
| DAgostini_Petit_2018_SNIa_AstrophysSpaceSci.pdf | 1.2 MB | JANUS-Z | OK |
| Petit_2025_Rebuttal_Damour_Criticism.pdf | 12.2 MB | JANUS-Z | OK |

**Articles importants ajoutés:**
1. **D'Agostini & Petit (2018)** - "Constraints on Janus Cosmological model from recent observations of supernovae type Ia" - Astrophysics & Space Science 363:139 - **12 confirmations observationnelles**
2. **Petit (2025)** - "Rebuttal of Damour's criticism of the Janus model" - Réponse aux critiques mathématiques de T. Damour

**Total publications: 13 PDFs + 1 HTML**

---

### Publications à rechercher (TODO)

#### Publications de référence manquantes:
- [x] ~~D'Agostini & Petit (2018) - Astrophysics & Space Science 363:139 (Supernovae Ia)~~ TROUVÉ
- [ ] Petit & D'Agostini (2015) - Astrophysics & Space Science 357:67 (Lagrangian)
- [ ] Petit & D'Agostini (2015) - Modern Physics Letters A 30(9) (Schwarzschild)
- [ ] Petit (1977) - Comptes rendus Académie des Sciences (article original)
- [ ] Petit & Zejli (2024) - Reviews in Mathematical Physics (groupes symplectiques)

#### Publications critiques à rechercher:
- [x] ~~Critique mathématique de Thibault Damour (2022)~~ RÉPONSE TROUVÉE
- [ ] Article de Sabine Hossenfelder (2008) sur les modèles bimétriques
- [ ] Autres critiques académiques publiées

---

### Prochaines étapes

1. Compléter la collection de publications
2. Télécharger les données observationnelles (SNIa, CMB, BAO)
3. Implémenter les scripts de validation
4. Comparer les prédictions du modèle aux observations

---

## 2025-01-05 - Analyse des publications V1-V13

### Versions analysées

| Version | Fichier | Pages | Statut |
|---------|---------|-------|--------|
| V1 | janus_v1_first_results.pdf | 9 | Analysé |
| V2 | janus_v2_correct_physics.pdf | 5 | Analysé |
| V6 | JANUS_v6_rigorous.pdf | 4 | Analysé |
| V9 | janus_v9_population_statistics.pdf | 4 | Analysé |
| V13 | janus_v13_final_consolidated.pdf | 7 | Analysé |

**Note**: V3-V5, V7-V8, V10-V12 non disponibles en PDF.

### Erreurs critiques détectées

| Version | Erreur | Gravité |
|---------|--------|---------|
| V6 | Symboles ■ corrompus (encodage) | CRITIQUE |
| V9 | Références "??" non résolues | CRITIQUE |
| V1 | Références bibliographiques erronées | MAJEURE |
| V13 | Aucune figure | MAJEURE |

### Document de spécifications créé

**Fichier**: `JANUS-Z/PUBLICATION_STANDARDS.md`

Règles de vérification de forme avant génération PDF:
- Structure obligatoire (sections, annexes)
- Uniformité linguistique (anglais)
- Format références bibliographiques (AAS style)
- Équations et symboles JANUS
- Checklist rapide (10 points)

---

*Dernière mise à jour: 2025-01-05*
